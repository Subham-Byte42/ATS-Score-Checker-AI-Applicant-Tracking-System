...
interface AuthModalProps {
  ...
  targetRole?: 'personal' | 'recruiter';
  onRoleChange?: (role: 'personal' | 'recruiter') => void;
  onClose: () => void;
  onLoginSuccess?: (user: { name: string; email: string; role?: 'personal' | 'recruiter'; companyName?: string }) => void;
}
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const [modalRole, setModalRole] = useState<'personal' | 'recruiter'>(targetRole);
  ...
  // Role mismatch prompt when user tries to log in with an ID belonging to another role
  const [roleMismatchPrompt, setRoleMismatchPrompt] = useState<{
    registeredRole: 'personal' | 'recruiter';
    targetRole: 'personal' | 'recruiter';
    message: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setModalRole(targetRole || 'personal');
      setIsLoading(false);
      setSubmitError('');
      setSignupSuccessMsg('');
      setResetSuccessMsg('');
      setRoleMismatchPrompt(null);
      setEmail('');
      setPassword('');
      setConfirmPassword('');
      setFullName('');
    ...
    const handleMessage = async (event: MessageEvent) => {
      if (event.data?.type === 'GOOGLE_AUTH_SUCCESS') {
        ...
        try {
          await fetch('/api/auth/signup', {
            ...
            body: JSON.stringify({
              ...
              role: modalRole,
              companyName: modalRole === 'recruiter' ? companyName : undefined,
            })
          });
        } catch (err) {
        ...
        try {
          ...
          if (!loginRes.ok && loginData.needsNewAccountForRole) {
            setRoleMismatchPrompt({
              registeredRole: loginData.registeredRole,
              targetRole: loginData.targetRole,
              message: loginData.message
            });
            setSubmitError(loginData.message);
            setIsLoading(false);
            return;
          }
          if (loginData.token) {
        ...
        setTimeout(() => {
          ...
          setIsLoading(false);
          if (onLoginSuccess) {
            onLoginSuccess({ name: derivedName, email: googleUserEmail, role: modalRole });
          }
          onClose();
        }, 800);
      }
  ...
  }, [onLoginSuccess, onClose, modalRole, companyName]);
    ...
    if (mode === 'signup') {
      ...
      try {
        const response = await fetch('/api/auth/signup', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: fullName || (email ? email.split('@')[0] : (modalRole === 'recruiter' ? 'Recruiter' : 'Candidate')),
            email,
            password,
            role: modalRole,
            companyName: modalRole === 'recruiter' ? companyName : undefined,
        ...
        setSubmitError('');
        setSignupSuccessMsg(
          `${modalRole === 'recruiter' ? 'Recruiter' : 'Personal Candidate'} ID created successfully! A confirmation welcome email has been sent. Please enter your password to log in.`
        );
        setMode('login');
        return;
      } catch (err: any) {
      ...
      try {
        ...
        if (!response.ok) {
          if (data.needsNewAccountForRole) {
            setRoleMismatchPrompt({
  ...
  const handleSelectGoogleAccount = async (selectedEmail: string, customName?: string) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      if (onLoginSuccess) {
        onLoginSuccess({ name: derivedName, email: selectedEmail, role: modalRole });
      }
      onClose();
    }, 800);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Previous Button to get back */}
        {!isGooglePickerOpen && (
          <button
            type="button"
            onClick={handlePrevious}
            className="absolute top-3.5 left-3.5 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-200/90 shadow-2xs transition-all cursor-pointer z-20 active:scale-98 group"
            title="Return to previous page"
          >
            <Undo2 className="w-3.5 h-3.5 text-slate-500 group-hover:text-blue-600 transition-colors" />
            <span>Previous</span>
          </button>
        )}
        ...
        {isGooglePickerOpen ? (
          ...
          <>
            {/* Modal Top Header */}
            {mode === 'forgot' ? (
              <div className="bg-gradient-to-b from-blue-50/70 to-white p-4 pt-12 pb-3 border-b border-slate-100 text-center relative">
              ...
              <div className={`p-4 pt-12 pb-3 border-b border-slate-100 text-center relative ${
                (targetRole === 'recruiter' || modalRole === 'recruiter')
                  ? 'bg-gradient-to-b from-indigo-50/70 to-white'
                ...
                <h3 className="text-lg sm:text-xl font-bold text-slate-900">
                  {(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Recruiter Portal Log In' : 'Personal Workspace Log In'}
                </h3>
                <p className="text-xs text-slate-600 mt-0.5 font-normal">
                  {(targetRole === 'recruiter' || modalRole === 'recruiter') 
                    ? 'Enter your dedicated Recruiter ID credentials' 
              ...
              <div className={`p-4 pt-12 pb-3 border-b border-slate-100 text-center relative ${
                ...
                <h3 className="text-lg sm:text-xl font-bold text-slate-900">
                  {(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Create Recruiter ID' : 'Create Personal Candidate ID'}
                </h3>
                <p className={`text-sm font-bold mt-0.5 ${(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'text-indigo-600' : 'text-[#3B82F6]'}`}>
                  {(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Dedicated ID for HR & Recruiters' : 'Create Free Candidate Account'}
                </p>
                <p className="text-[11px] text-slate-600 mt-0.5 font-normal">
                  {(targetRole === 'recruiter' || modalRole === 'recruiter')
                    ? 'Dedicated Recruiter ID for applicant tracking, JD matching, and candidate screening.'
                    : 'Analyze resume match scores, identify skill gaps, and optimize for ATS.'}
                </p>
              </div>
            )}

            {/* Segmented Role Switcher & Role Guidance */}
            {(mode === 'login' || mode === 'signup') && (
              <div className="px-4 pt-3 pb-1 space-y-2">
                {/* Role Header Badge - Recruiter portal shows recruiters ONLY with Personal ID deleted */}
                {(targetRole === 'recruiter' || modalRole === 'recruiter') ? (
                  <div className="flex items-center justify-center gap-2 py-2 px-3 bg-indigo-50/90 rounded-xl text-xs font-bold border border-indigo-200 text-indigo-800 shadow-2xs">
                    <Building2 className="w-4 h-4 text-indigo-600 shrink-0" />
                    <span>Recruiter Portal • Dedicated Recruiter ID</span>
                  </div>
                ) : (
                  <div className="flex items-center justify-center gap-2 py-2 px-3 bg-blue-50/90 rounded-xl text-xs font-bold border border-blue-200 text-blue-800 shadow-2xs">
                    <User className="w-4 h-4 text-blue-600 shrink-0" />
                    <span>Personal Workspace • Candidate ID</span>
                  </div>
                )}

                {/* Role Guidance Pill */}
                <div className={`p-2 rounded-lg text-[11px] border flex items-center gap-2 ${
                  (targetRole === 'recruiter' || modalRole === 'recruiter')
                  ...
                  {(targetRole === 'recruiter' || modalRole === 'recruiter') ? (
                    <>
                      ...
                      <span><strong>Recruiter Portal:</strong> Sign in or register with your Recruiter ID for candidate screening.</span>
                    </>
                  ) : (
                    <>
                      <UserCheck className="w-3.5 h-3.5 text-blue-600 shrink-0" />
                      <span><strong>Personal Workspace:</strong> For job seekers tracking and optimizing their own resumes.</span>
                    </>
                  )}
                </div>

                {/* Role Mismatch Guidance Alert if cross-role login was attempted */}
                {roleMismatchPrompt && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs space-y-2 animate-in fade-in">
                    <div className="flex items-start gap-2">
                      <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                      <div>
                        <p className="font-bold text-amber-950">
                          {roleMismatchPrompt.registeredRole === 'personal'
                            ? 'Email is registered for Personal Candidate ID'
                            : 'Email is registered for Recruiter ID'}
                        </p>
                        <p className="text-[11px] text-amber-800 mt-0.5 leading-relaxed">
                          {roleMismatchPrompt.message}
                        </p>
                      </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 pt-1">
                      <button
                        ...
                        className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold text-xs flex items-center justify-center gap-1 cursor-pointer transition-colors shadow-xs"
                      >
                        <span>Create {modalRole === 'recruiter' ? 'Recruiter' : 'Personal'} ID Now</span>
                      ...
                      {!(targetRole === 'recruiter' || modalRole === 'recruiter') && (
                        <button
                          type="button"
                          onClick={() => {
                            const switchTarget = roleMismatchPrompt.registeredRole;
                            setModalRole(switchTarget);
                            setRoleMismatchPrompt(null);
                            setSubmitError('');
                            if (onRoleChange) onRoleChange(switchTarget);
                          }}
                          className="px-3 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg font-semibold text-xs flex items-center justify-center cursor-pointer transition-colors"
                        >
                          <span>Switch to {roleMismatchPrompt.registeredRole === 'recruiter' ? 'Recruiter' : 'Personal'} ID Log In</span>
                        </button>
                      )}
                    </div>
                  </div>
            ...
            {mode === 'forgot' ? (
              ...
              <form onSubmit={handleSubmit} className="p-4 space-y-2.5">
                      ...
                      <div className="relative">
                        ...
                        <label 
                          ...
                          className="absolute left-9 top-1/2 -translate-y-1/2 text-slate-400 text-xs transition-all pointer-events-none bg-transparent px-1
                                     ...
                                     peer-not-placeholder-shown:-top-2 peer-not-placeholder-shown:translate-y-0 peer-not-placeholder-shown:text-[10px] peer-not-placeholder-shown:font-semibold peer-not-placeholder-shown:text-slate-600 peer-not-placeholder-shown:bg-white"
                        >
                          {modalRole === 'recruiter' ? 'Recruiter / Hiring Lead Name' : 'Enter Candidate Full Name'}
                        </label>
                      </div>
                    </div>

                    {/* Optional Company / Organization field for Recruiter mode */}
                    {modalRole === 'recruiter' && (
                      <div className="relative pt-1.5">
                        <div className="relative">
                          <Building2 className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 z-10 pointer-events-none" />
                          <input
                            id="companyName"
                            type="text"
                            value={companyName}
                            ...
                            placeholder=" "
                            className="peer w-full pl-9 pr-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-normal text-slate-900 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all outline-hidden"
                          />
                          <label 
                            htmlFor="companyName" 
                            className="absolute left-9 top-1/2 -translate-y-1/2 text-slate-400 text-xs transition-all pointer-events-none bg-transparent px-1
                                       peer-focus:-top-2 peer-focus:translate-y-0 peer-focus:text-[10px] peer-focus:font-semibold peer-focus:text-indigo-600 peer-focus:bg-white
                                       peer-not-placeholder-shown:-top-2 peer-not-placeholder-shown:translate-y-0 peer-not-placeholder-shown:text-[10px] peer-not-placeholder-shown:font-semibold peer-not-placeholder-shown:text-slate-600 peer-not-placeholder-shown:bg-white"
                          >
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full py-2 disabled:opacity-70 disabled:cursor-not-allowed active:scale-[0.99] text-white font-semibold text-xs sm:text-sm rounded-lg shadow-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer mt-1 ${
                    (targetRole === 'recruiter' || modalRole === 'recruiter')
                      ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/20'
                      : 'bg-[#1877f2] hover:bg-[#0866ff]'
                  }`}
                >
                  {isLoading ? (
                    <>
                      ...
                      <span>
                        {mode === 'login' 
                          ? 'Signing In...' 
                          : ((targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Creating Recruiter ID...' : 'Creating Account...')}
                      </span>
                    </>
                  ) : (
                    <>
                      <span>
                        {mode === 'login' 
                          ? ((targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Sign In with Recruiter ID' : 'Sign In') 
                          : ((targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Create Recruiter ID' : 'Create Free Account')}
                      </span>
                      <ArrowRight className="w-3.5 h-3.5" />
                    </>
                  )}
                    ...
                    <span>
                      ...
                      <button
                        ...
                        className={`font-semibold hover:underline cursor-pointer ${
                          (targetRole === 'recruiter' || modalRole === 'recruiter') ? 'text-indigo-600' : 'text-[#1877f2]'
                        }`}
                      >
                        {(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Create a Recruiter ID' : 'Create a free account'}
                      </button>
                    </span>
                  ) : (
                    <span>
                      ...
                      >
                        {(targetRole === 'recruiter' || modalRole === 'recruiter') ? 'Log In with Recruiter ID' : 'Log In'}
                      </button>
                    </span>
                  )}
                </div>
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-center">
                  <button
                    type="button"
                    onClick={handlePrevious}
                    className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors cursor-pointer py-1.5 px-3 rounded-lg hover:bg-slate-100"
                    title="Return to previous page"
                  >
                    <Undo2 className="w-3.5 h-3.5 text-slate-400 group-hover:text-slate-700" />
